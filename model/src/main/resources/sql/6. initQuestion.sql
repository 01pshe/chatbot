...
VALUES (nextval('hibernate_sequence'), 'ObjIntConsumer<T>', 'ObjLongConsumer<T>', 'Predicate<T>', 'Supplier<T>', 'D',
        'Supplier<T>', 'Which of the following functional interface represents a supplier of results', 1, 'DIFFICULT');
...
VALUES (nextval('hibernate_sequence'), 'false', 'true', '', '', 'B', 'true',
        'A method reference is described using :: (double colon) symbol.', 1, 'DIFFICULT');

INSERT INTO public.bot_question (id, answera, answerb, answerc, answerd, answerr, answerright, question, useday, weight)
VALUES (nextval('hibernate_sequence'), 'false', 'true', '', '', 'B', 'true',
        'Stream operations do the iterations internally over the source elements provided.', 1, 'DIFFICULT');
...
VALUES (nextval('hibernate_sequence'), '(int a, int b) -> a + b;', '(a, b) -> a + b', 'Both of the above.',
        ...
        'Which of the following is the correct lambda expression which add two numbers and return their sum?', 2,
        'DIFFICULT');
